...
This stage tests whether `AD-SRV01` can apply a computer-side Group Policy setting to a domain-joined Windows 10 workstation.

The visible test is a logon notice on `AD-WIN10-01`. The notice itself is simple, but it proves the full policy path: the workstation object sits in the correct OU, the GPO is linked to that OU, the client refreshes policy, `gpresult` shows the applied GPO, and Windows displays the expected result.
...
The logon notice setting sits under Security Options in Group Policy Management.
